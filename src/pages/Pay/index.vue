...
export default {
  ...
  setup() {
    ...
    const { orderid, totalfee } = route.query;
    const payCode = ref();
    const open = async () => {
      let timer = null;
      timer = setInterval(async () => {
        ...
        // console.log(result);
        payCode.value = result.code;
      }, 1000);
      await reqPayInfo(orderid).then((res) => {
        QRCode.toDataURL(res.data.codeUrl).then((url) => {
          ElMessageBox.alert(`<img src="${url}"></img>`, {
