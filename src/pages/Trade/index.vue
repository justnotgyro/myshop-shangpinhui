  ...
  <div class="trade-container">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        ...
        收货人：<span>{{ userDefaultAddress?.fullAddress }}</span>
...
export default {
  ...
  setup() {
    ...
    store.dispatch("addressInfo");
    store.dispatch("orderInfo");
    const addressInfo = computed(() => store.state.trade.addressInfo);
    const orderInfo = computed(() => store.state.trade.orderInfo);
    const changeDefault = (index) => {
      addressInfo.value.forEach((item) => {
        item.isDefault = "0";
      });
      addressInfo.value[index].isDefault = "1";
    };
    const userDefaultAddress = computed(() =>
      addressInfo.value.find((item) => item.isDefault === "1")
    );
    const detailArrayList = computed(
      () => store.state.trade.orderInfo.detailArrayList
    );
    const message = ref("");
    const submitOrder = async () => {
      // 此处的订单数据是通过post存进服务器
      // 并在pay界面发送请求获取上述数据使用
      // 这样就不需要通过兄弟组件传值或者vuex仓库来共享数据了
      // 因为后端已经写好了发送和获取的接口~妙哉
      let data = {
        tradeNo: orderInfo.value.tradeNo,
        consignee: userDefaultAddress.value.consignee,
        consigneeTel: userDefaultAddress.value.consignee,
        deliveryAddress: userDefaultAddress.value.fullAddress,
        paymentWay: "ONLINE",
        orderComment: message.value,
        orderDetailList: detailArrayList.value,
      };
      await reqSubmitOrder(data).then((res) => {
        console.log(res);
        if (res.code === 200) {
          router.push({
            path: "/pay",
            query: {
              orderid: res.data,
              totalfee: orderInfo.value.totalAmount,
            },
          });
        } else {
          alert("提交订单失败！");
